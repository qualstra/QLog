...
//import com.enoch.serviceregistration.eureka.client.Attribute;
...
@Component
@Data
public class CASReport implements Report{ 
	...
	public String report(Context cont) {
		...
		return "deflt/detail/detailReport"; 
	}
	...
	public CheckListInstVO populate(CheckListInstDTO checklistDTO) {
		...
		try {
			...
			final List<String> top5Section = getTop5Section((JSONArray) sumarry.get("sections")); 
			this.file = File.createTempFile("CHART", "png", new File(props.getResourceFolder()));
			BarChartGenerator.getCHart(checklistDTO.getVessel().getName() + " CAS Scores", "Section", "Score", () -> {
			...
			LineChartGenerator.getCHart(checklistDTO.getVessel().getName() + " CAS Scores comparison", "Checklist",
					"Score", () -> {
						DefaultCategoryDataset dataset = new DefaultCategoryDataset();
	...
	public boolean checkNCForOption(QuestionInstVO questionInstDTO) {
		//handler for answer is null
		if(questionInstDTO.getAnswer() == null) 
			return false;

		//handler for \n \t
		questionInstDTO.setAnswer(questionInstDTO.getAnswer().replace('\n', ' '));
		questionInstDTO.setData(questionInstDTO.getData().replace('\n', ' '));
...
//		System.out.println(attr.getValue());
	...
	private List<String> getTop5Section(JSONArray secData) {
		ArrayList<JSONObject> listdata = new ArrayList<JSONObject>();
		if (secData != null) {
			for (int i = 0; i < secData.length(); i++) {
				listdata.add((JSONObject) secData.get(i));
		...
		return listdata.subList(0, listdata.size() > 5 ? 5 : listdata.size()).stream().map(sec -> sec.getString("name")).collect(Collectors.toList());
	}
	protected final Log logger = LogFactory.getLog(CASReport.class);
